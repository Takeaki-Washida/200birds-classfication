...
def compute_saliency(model, guided_model, img_path, layer_name='block5_conv3', cls=-1, visualize=True, save=True):
    ...
    for file_name in file_list:
        ...
        if save:
            ...
            #領域削減用に鳥領域座標を取得
            x_min = object_index[0][0]
            x_max = object_index[0][0]
            y_min = object_index[0][1]
            y_max = object_index[0][1]

            for i in range(150):
                for m in range(150):
                    #grad-camで赤マッピングされている位置
            ...
            #鳥領域に注目
            if obj_count_up >= 10:
                print("looking at bird!")
                i_max = 1

            #背景領域に注目
            elif obj_count_up < 10:
                print("looking at back!")
                i_max = 0

                #領域削減--------------------------------
                for n in range(len(object_index)):
                    if object_index[n][0] < x_min:
                        x_min = object_index[n][0]
                    if object_index[n][0] > x_max:
                        x_max = object_index[n][0]
                    if object_index[n][1] < y_min:
                        y_min = object_index[n][1]
                    if object_index[n][1] > y_max:
                        y_max = object_index[n][1]

                x_min_ori = x_min * w_rate
                x_max_ori = x_max * w_rate
                y_min_ori = y_min * h_rate
                y_max_ori = y_max * h_rate

                crop_xleft = x_min_ori - 20
                crop_xright = x_max_ori + 20
                crop_yup = y_min_ori - 20
                crop_ydown = y_max_ori + 20

                if crop_xleft < 0:
                    while crop_xleft < 0:
                        crop_xleft += 1
                if crop_xright > ori_w:
                    while crop_xright > ori_w:
                        crop_xright -= 1
                if crop_ydown > ori_h:
                    while crop_ydown > ori_h:
                        crop_ydown -= 1
                if crop_yup < 0:
                    while crop_yup < 0:
                        crop_yup += 1
